...
@Component
public class ActivityTemplateDataInfoPresetCriteriaMaker implements PresetCriteriaMaker {

    @Override
    public void makeCriteria(DetachedCriteria detachedCriteria, String s, Object[] objects) {
        switch (s) {
            case ActivityTemplateDataInfoMaintainService.CHILD_FOR_ACTIVITY_TEMPLATE:
                childForActivityTemplate(detachedCriteria, objects);
                break;
            case ActivityTemplateDataInfoMaintainService.CHILD_FOR_ACTIVITY_DATA_ITEM:
    ...
    private void childForActivityDataItem(DetachedCriteria detachedCriteria, Object[] objects) {
        try {
            if (Objects.isNull(objects[0])) {
                detachedCriteria.add(Restrictions.isNull("activityDataItemLongId"));
            } else {
                LongIdKey longIdKey = (LongIdKey) objects[0];
                detachedCriteria.add(
                        Restrictions.eqOrIsNull("activityDataItemLongId", longIdKey.getLongId())
                );
            }
        } catch (Exception e) {
            throw new IllegalArgumentException("非法的参数:" + Arrays.toString(objects));
