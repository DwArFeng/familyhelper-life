...
import com.dwarfeng.familyhelper.life.sdk.bean.key.FastJsonPoacKey;
...
public class FastJsonPoac implements Bean {
    ...
    private static final long serialVersionUID = -2935043499295813690L;

    public static FastJsonPoac of(Poac poac) {
        if (Objects.isNull(poac)) {
            return null;
        } else {
            return new FastJsonPoac(
                    FastJsonPoacKey.of(poac.getKey()),
                    poac.getPermissionLevel(), poac.getRemark()
            );
        }
    }
